    ...
    results.then(function(results) {
      ...
      if($scope.query_id === query_id) {
        ...
        $scope.plotseries[0] = {
            data: [],
            hits: 0,
            info: $scope.queries.list[0]
        };
        $scope.plotseries[0].info.alias=$scope.panel.value_field;
        // Get printed value
        _.each($scope.data, function(item, index) {
          // pas convaincu de cette manière de récupérer la valeur
          if (typeof item._source[$scope.panel.value_field] !== 'undefined' && item._source[$scope.panel.value_field] !== null) {
            // Timestamp parsing
            var parts = item._source[$scope.panel.time_field].split('T'),
            ...
            $scope.plotseries[0].data[i] = [_date.getTime(), item._source[$scope.panel.value_field]];
            $scope.plotseries[0].hits += 1;
            i++;
          }
        });
        $scope.truehits=i;
